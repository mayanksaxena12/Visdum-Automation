 ...
 *
 * <p>The list is an AG-Grid with pinned columns, so a single visual row is split across separate
 * DOM containers (center vs. pinned-right), each with its own {@code role='row'} element sharing the
 * same {@code row-index} attribute. The name/email cells live in the center container while the
 * action button ({@code #userAction{index}}) and status badge live in the pinned-right container.
 * We therefore locate a user by the text in the center row, read its {@code row-index}, and target
 * the matching pinned cells by that index.
 */
